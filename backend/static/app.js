...
(function () {
  ...
  /* ================= icons + badges (designer system) ================= */
  function icon(name) {
    var icons = {
      dashboard: "▦", workspace: "⬡", billing: "▣", support: "?", account: "◯", overview: "▦",
      requests: "♧", accounts: "◎", maintenance: "↻", settings: "⚙", bell: "♢", menu: "☰",
      check: "✓", warning: "!", error: "×", clock: "◷", lock: "▣", copy: "▢", plus: "+",
      external: "↗", shield: "◇", mail: "✉", server: "▤", more: "⋮", arrow: "→", spark: "✦"
    };
    ...
    return '<div class="app-shell">' +
      ...
      '<div class="main-area">' +
        '<header class="topbar"><div class="actions">' +
          ...
          "<h2>" + esc(title) + "</h2></div>" +
          '<div class="top-actions"><button class="top-icon" data-action="signout-route" aria-label="Sign out" title="Sign out">' + icon("account") + "</button></div>" +
        "</header>" + content +
      "</div></div>";
  }
  ...
  function registerPage() {
    ...
    return authLayout(
      ...
      '<form data-form="register"><div class="form-grid">' +
        ...
        '<button type="button" class="input-action" data-action="toggle-password" data-target="reg-password" aria-label="Show password">◉</button></div></div>' +
        '<div class="field"><label>Confirm password</label><div class="input-row">' +
        '<input id="reg-confirm" type="password" required>' +
        '<button type="button" class="input-action" data-action="toggle-password" data-target="reg-confirm" aria-label="Show password">◉</button></div></div>' +
        '<div class="checks full" id="pw-checks">' +
        '<span class="check" data-check="len">At least 8 characters</span>' +
        '<span class="check" data-check="upper">One uppercase letter</span>' +
        '<span class="check" data-check="lower">One lowercase letter</span>' +
        '<span class="check" data-check="digit">One digit</span>' +
        '<span class="check" data-check="match">Passwords match</span></div>' +
      '</div>' +
  ...
  function signinPage() {
    return authLayout(
      ...
      '<button type="button" class="input-action" data-action="toggle-password" data-target="signin-password">◉</button></div></div>' +
  ...
  function dashboardStatus() {
    ...
    if (s === "active") {
      ...
      return '<div class="hero-status"><span class="big-icon">✓</span><div><h2>Nothing needs your attention</h2><p>Your subscription and workspace are running normally.</p></div>' +
        '<a class="button" href="' + esc(url) + '" target="_blank" rel="noopener">Open workspace ' + icon("external") + "</a></div>";
    }
    if (s === "provisioning") {
      return '<div class="hero-status info"><span class="big-icon">◷</span><div><h2>Your workspace is being prepared</h2><p>This usually takes a few minutes. We will email you when it is ready.</p></div></div>';
    }
    if (s === "failed") {
      return '<div class="hero-status danger"><span class="big-icon">!</span><div><h2>Setup needs attention</h2><p>Something went wrong. SteProTECH has been notified and will contact you.</p></div>' +
        '<a class="button danger" href="mailto:support@steprotech.com">Contact support</a></div>';
    }
    if (s === "pastdue") {
      return '<div class="hero-status warning"><span class="big-icon">!</span><div><h2>Your renewal payment did not go through</h2><p>Retry before ' + esc(fmtDate(acc.paid_until)) + " to prevent your workspace from being switched off.</p></div>" +
        '<button class="button warning" data-action="pay-now">Retry payment</button></div>';
    }
    if (s === "cancelled") {
      return '<div class="hero-status warning"><span class="big-icon">!</span><div><h2>Your subscription is cancelled</h2><p>Your workspace remains available until the paid period ends on ' + esc(fmtDate(acc.paid_until)) + ".</p></div>" +
    ...
    return '<div class="hero-status info"><span class="big-icon">▣</span><div><h2>Choose a plan to get started</h2><p>Subscribe to the annual plan and your private workspace is created after payment.</p></div>' +
  ...
  function customerWorkspaces() {
    ...
    var detail = "";
    if (s === "provisioning") {
      detail = '<section class="card" style="margin-top:18px"><div class="hero-status info"><span class="big-icon">◷</span><div><h2>Your workspace is being prepared</h2><p>You can close this page. We will email you the moment it is ready.</p></div></div>' +
        '<div class="progress-steps"><div class="step done"><span class="step-dot">✓</span>Order received</div><div class="step active"><span class="step-dot">2</span>Preparing workspace</div><div class="step"><span class="step-dot">3</span>Configuring services</div><div class="step"><span class="step-dot">4</span>Final checks</div></div>' +
    ...
    var planCards = plans.map(function (p) {
      ...
      if (activePlan) {
        ...
        return '<article class="plan-card featured"><div class="eyebrow">Available now</div><h2>' + esc(p.name) + "</h2>" +
          ...
          '<ul class="feature-list"><li>Your own private workspace</li><li>Your own secure address</li><li>Email support</li><li>Automatic annual renewal</li></ul>' +
  ...
  function adminSignin() {
    return authLayout(
      ...
      '<button type="button" class="input-action" data-action="toggle-password" data-target="admin-password">◉</button></div></div>' +
      '<button class="button primary-wide" type="submit">Sign in</button></form>' +
    ...
    return appLayout("admin", "Overview", "overview",
      ...
      '<div class="grid cols-4">' +
        '<article class="card metric"><div class="metric-top"><span class="metric-icon">' + icon("requests") + "</span>" + statusBadge("awaiting", "Needs review") + "</div>" +
          '<div class="metric-value">' + awaiting + '</div><p class="muted">Access requests awaiting approval</p><a href="#/admin/requests">Review requests →</a></article>' +
        '<article class="card metric"><div class="metric-top"><span class="metric-icon" style="background:var(--amber-100);color:var(--amber-700)">' + icon("warning") + "</span></div>" +
          '<div class="metric-value">' + pastDue + '</div><p class="muted">Accounts past due</p><a href="#/admin/accounts">View accounts →</a></article>' +
        '<article class="card metric"><div class="metric-top"><span class="metric-icon" style="background:var(--green-100);color:var(--green-700)">' + icon("workspace") + "</span></div>" +
          '<div class="metric-value">' + locked + '</div><p class="muted">Locked / cancelled accounts</p><a href="#/admin/accounts">Review →</a></article>' +
        '<article class="card metric"><div class="metric-top"><span class="metric-icon" style="background:var(--slate-100);color:var(--slate-700)">' + icon("account") + "</span></div>" +
          '<div class="metric-value">' + total + '</div><p class="muted">Total customer accounts</p><a href="#/admin/accounts">View accounts →</a></article>' +
      "</div>" +
      '<div class="grid cols-3" style="margin-top:18px">' +
  ...
  function settingsPage() {
    ...
    var rows = order.map(function (id, i) {
      ...
      return '<div class="server"><span class="drag">⠿</span><strong>' + (i + 1) + "</strong>" +
        ...
        '<button data-server-up="' + i + '" aria-label="Move up">↑</button>' +
        '<button data-server-down="' + i + '" aria-label="Move down">↓</button></span></div></div>';
  ...
  function showToast(title, message) {
    ...
    el.className = "toast";
    el.innerHTML = '<span class="toast-icon">✓</span><div><strong>' + esc(title) + "</strong><p>" + esc(message) + "</p></div>";
  ...
  function modalHeader(title) {
    return '<div class="modal-head"><h2 id="modal-title">' + esc(title) + '</h2><button class="modal-close" data-action="close-modal" aria-label="Close">×</button></div>';
  }
  ...
  function approvalSuccess(data) {
    ...
    showModal('<div class="center-state"><div class="state-orb success">✓</div><h2 id="modal-title">Access approved</h2>' +
  ...
  async function autoProvision() {
    ...
    if (!pw) {
      ...
      showModal(modalHeader("Provision your workspace") +
        ...
        '<button type="button" class="input-action" data-action="toggle-password" data-target="provision-pw" aria-label="Show password">◉</button></div>' +
  ...
  function eventHandlers() {
    document.addEventListener("click", function (event) {
      ...
      else if (action === "toggle-password") { var inp = document.getElementById(act.dataset.target); if (inp) inp.type = inp.type === "password" ? "text" : "password"; }
      ...
      /* admin */
        ...
        adminAction("/admin/billing/sweep-expired", "Maintenance complete", null).then(function (data) {
          if (data) {
            ...
            showModal('<div class="center-state"><div class="state-orb success">✓</div><h2 id="modal-title">' + n + " workspace(s) switched off</h2>" +
    ...
    document.addEventListener("input", function (event) {
      ...
      if (event.target.id === "reg-password" || event.target.id === "reg-confirm") {
        ...
        var set = function (name, ok) {
          ...
          if (el) { el.className = "check" + (ok ? " ok" : ""); el.innerHTML = (ok ? "✓ " : "") + el.dataset.label; }
        };
